function x(e,i,p,o){let n=o?.companyName||"Sepab Agro",d=n.split(" ")[0],r=o?.companyAddress||"Rue Farhat Hached, Morneg, Ben Arous",s=o?.companySector||"",c=o?.companyEmail||"",l=o?.companyPhone||"",a=o?.companyLogo||null,g=a?`<img src="${a}" style="height:48px;width:48px;object-fit:contain;border-radius:8px;flex-shrink:0" alt="${n}">`:"",m=[`<strong>${n}</strong>`,r,s,c,l].filter(Boolean).join(' <span style="color:#CBD5E1;margin:0 6px">\xB7</span> '),f=`<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<title>${e} \u2014 PointagePro</title>
  ...
  <div class="content">
    <div class="header">
      <div class="header-left">
        ${g}
        <div>
          <h3 class="company-name">${d}</h3>
          <p class="company-details">${n} \u2014 ${r}</p>
        </div>
      </div>
      <div class="header-right">
        <span class="doc-type">${e}</span>
        <span class="doc-period">P\xE9riode : ${i}</span>
        <span class="doc-ref">G\xE9n\xE9r\xE9 le ${new Date().toLocaleDateString("fr-FR")}</span>
      </div>
    </div>
...
</html>`,t=window.open("","_blank","width=900,height=700");t&&(t.document.write(f),t.document.close())}export{x as a};
