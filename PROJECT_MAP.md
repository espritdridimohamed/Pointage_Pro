...
├── employee/          # Employee entity/repository + EmployeeService + EmployeeRecordService (documents/bank accounts/dependents/emergency contacts/tax profiles/assignments) (module 6 DONE)
├── organization/      # Department/Position/Location entities + OrganizationService + controllers (module 6 DONE)
├── contract/          # EmployeeContract/SalaryComponent entities + ContractService + Controller (module 6 DONE)
├── schedule/          # WorkSchedule/EmployeeSchedule entities + ScheduleService + controllers (module 6 DONE)
├── lookup/            # Read-only dropdown endpoints (/lookups/...) (module 6 DONE)
...
**DB Migrations:** V1–V16 all applied
...
- **Module 6 FROZEN 2026-08-06 — full verification passed:** `mvn clean verify` BUILD SUCCESS (232 tests, 0 failures/errors/skipped, 18 test classes); fresh startup clean (Flyway V1–V16 up to date — **V16** applied live: relaxes department-name uniqueness via STORED `active_name` + `uk_departments_company_code`); live E2E vs MySQL (JWT for `admin`/`admin000`): org create (department/position/location) → flat employee create (auto matricule `EMP-001`, auto assignment + CDI contract + BASE 1500/transport 100/performance 200 + salary history) → Phase B (document; 2 bank accounts `isDefault=true` → 1 default; dependent; emergency contact; 2 tax profiles → first auto-closed) → contract list/components/salary-history → tax-profile rules (open stays open, back-dated open → 409, update-to-open while another open → 409) → `DELETE` terminate (sets `exit_date`, auto-closes open ACTIVE contract `EXPIRED`/`end_date`=today); DB integrity + cleanup (org unique per company, single ACTIVE contract, single default bank, single open tax profile; DB restored to admin-only). New: `contract`, `schedule`, `organization`, `lookup` packages + `EmployeeRecordService` + Phase B DTOs + `CurrentUserService` admin→DEFAULT-company fallback + payroll exit-date exclusion. Docs: `EMPLOYEE_BUSINESS_RULES.md`, `EMPLOYEE_API_CONTRACT.md` (APPROVED v0.2). Tagged `module-6`.
